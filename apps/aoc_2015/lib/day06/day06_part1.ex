defmodule Day06.Part1 do
  ...
  def do_instructions(instructions) do
    Enum.reduce(instructions, %{}, &(do_instruction_flow(&2, &1)))
  end

  ###

  def do_instruction_flow(state_map, {:on, x_range, y_range}) do
  ...
  def do_instruction_flow(state_map, {:off, x_range, y_range}) do
      get_locs_for_ranges(x_range, y_range)
      |> Flow.from_enumerable()
      # |> Flow.partition()
      |> Flow.reduce(fn -> state_map end, fn loc, acc ->
  ...
  def do_instruction_flow(state_map, {:toggle, x_range, y_range}) do
      get_locs_for_ranges(x_range, y_range)
      |> Flow.from_enumerable()
      # |> Flow.partition()
      |> Flow.reduce(fn -> state_map end, fn loc, acc ->
  ...
  ###
  # attempted with a parallel map implementation - appears to be junk
  # these ones are junk
  ...
  ### Original version - slow, deprecated in favor of do_instruction_merge
