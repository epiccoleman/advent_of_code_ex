...
################################################################################
  ...
  @doc """
  Final implementation before trying priority queue.
  """

  def dijkstra_5(grid, start_pos, end_pos) do
    # this function head is for the first time you call the thing
    distances = Enum.map(grid, fn {pos, _} ->
      {pos, 10_000_000_000}
    end)
    ...
    dijkstra_5(grid, start_pos, end_pos, state)
  end

  def dijkstra_5(grid, start_pos, end_pos, %{distances: distances, prev: prev, current: current, unvisited: unvisited}) do
    # mark the current node as visited
    # IO.puts("running for #{inspect(current)}")
    new_unvisited = MapSet.delete(unvisited, current)
    ...
    if new_current == end_pos do
      ...
      dijkstra_5(grid, start_pos, end_pos, new_state)
    end
  end
end
